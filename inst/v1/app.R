...
library(patchwork)
library(VIP)
source(system.file("v1", "functions.R", package = "HPV"))
...
# Meta-analysis data
meta <- data_studies[data_studies$is_meta == 1L, , drop = FALSE]
...
server <- function(input, output, session) {
  ...
  # Meta-analysis tab
  plot_meta <- metaAnalysisServer("meta", data = data)
}
